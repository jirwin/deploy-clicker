...
var updateUsers = function(io, results, callback) {
  ...
  async.parallel({
    userCount: function(callback) {
      redisClient.incrby(userCountKey, magnitude, function(err, reply) {
        io.sockets.emit('userCount', {userCount: reply});
        callback(err, reply);
      });
    },

    totalCount: function(callback) {
      redisClient.incrby(totalUsersKey, magnitude, function(err, reply) {
        io.sockets.emit('totalCount', {totalCount: reply});
        callback(err, reply);
      });
    },
